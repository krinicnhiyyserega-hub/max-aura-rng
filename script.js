...
// НАСТОЯЩАЯ ФУНКЦИЯ ПРОСМОТРА РЕКЛАМЫ ЯНДЕКСА
function watchAd() {
    // Проверяем, загрузилась ли библиотека Яндекса в браузере
    if (typeof window.yaContextCb === 'undefined') {
        alert("Рекламная сеть сейчас недоступна. Попробуйте отключить AdBlock или перезагрузить страницу.");
        return;
    }

    // Вызываем полноэкранный плеер Яндекса
    window.yaContextCb.push(() => {
        Ya.Context.AdvManager.render({
            blockId: 'R-A-XXXXXX-Y', // ⚠️ ЗАМЕНИТЕ ЭТОТ ID НА СВОЙ ИЗ КАБИНЕТА ЯНДЕКСА!
            type: 'rewarded',        // Тип рекламы — видео за вознаграждение
            
            // Массив обратных вызовов (колбэков)
            callbacks: {
                onOpen: () => {
                    console.log("📺 Рекламный ролик запущен. Игра приостановлена.");
                },
                onRewarded: () => {
                    // 🔥 ЭТОТ БЛОК СРАБОТАЕТ, ТОЛЬКО ЕСЛИ ИГРОК ПОСМОТРЕЛ ВСЁ ВИДЕО
                    console.log("🎉 Игрок досмотрел рекламу! Выдаем награду.");
                    
                    playerEnergy += 5; // Начисляем +5 энергии
                    updateEnergyUI();  // Обновляем циферки на главном экране
                    saveProgress();    // Сохраняем новую энергию в память устройства
