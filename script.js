...
// 📺 ОБНОВЛЕННАЯ ФУНКЦИЯ РЕКЛАМЫ (НАГРАДА ПРИ ЗАКРЫТИИ)
function watchAd() {
    ...
    window.yaContextCb.push(() => {
        Ya.Context.AdvManager.render({
            ...
            callbacks: {
                onOpen: () => {
                    console.log("📺 Рекламное окно открыто. Игра на паузе.");
                },
                onClose: () => {
                    // 🎉 НАГРАДА: Срабатывает сразу, как только игрок закрыл полноэкранный баннер
                    console.log("🎉 Реклама закрыта! Выдаем энергию и включаем таймер.");
                    
                    playerEnergy += 5; // Начисляем +5 энергии
                    updateEnergyUI();  // Обновляем текст на экране
                    saveProgress();    // Сохраняем в память устройства
                    
                    alert("🎉 Спасибо за просмотр! Вам начислено +5 энергии 🔋.");

                    // Включаем блокировку кнопки рекламы на 5 минут (300 секунд)
                    startAdCooldown(300);
                },
                onError: (error) => {
